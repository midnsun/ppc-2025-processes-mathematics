...
bool ZagryadskovMMaxByColumnMPI::RunImpl() {
  ...
  if (columns_size > 0) {
    ...
    if (world_rank == 0) {
      ...
      MPI_Scatter(nullptr, columns_size, datatype, columns.data(), columns_size, datatype, 0, MPI_COMM_WORLD);
    }
    ...
    if (world_rank == 0) {
      ...
      MPI_Gather(local_res.data(), columns_count, datatype, nullptr, columns_count, datatype, 0, MPI_COMM_WORLD);
    }
  }
  if (world_rank == 0) {
